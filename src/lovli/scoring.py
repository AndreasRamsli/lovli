"""Shared retrieval/scoring utilities to keep runtime and scripts in sync."""
...
def build_law_cross_reference_affinity(
    ...
    for law_id, rows in grouped.items():
        ...
        for row in rows:
            for raw_ref in row.get("cross_references") or []:
                resolved = _resolve_cross_reference_law_id(
                    raw_ref, law_ref_to_id, candidate_law_ids
                )
...
def build_law_coherence_decision(
    ...
    for item in law_stats[1:]:
        ...
        if abs_gap < min_abs_gap and relative_gap < min_rel_gap:
            ...
            foreign_tie_break = (
                (0.50 * law_concentration) + (0.30 * law_affinity) + (0.20 * law_consistency)
            )
...
def build_law_aware_rank_fusion(
    ...
    dominant_law_id = (dominant_context or {}).get("dominant_law_id") or (
        law_stats[0]["law_id"] if law_stats else ""
    )
    ...
    for row in candidates:
        ...
        concentration = (
            (law_count_map.get(law_id, 0) / total_candidates) if total_candidates else 0.0
        )
