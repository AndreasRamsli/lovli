...
def create_citation_match_evaluator(settings: Settings):
    ...
    def citation_match_evaluator(
        ...
        matches = [cid for cid in cited_articles if matches_expected(cid, expected_articles)]
        ...
        try:
            if hasattr(experiment_results, "results") and experiment_results.results:
                            ...
                            if feedback and isinstance(feedback, dict):
                                ...
                                if score is not None:
                                    ...
                                    score_val = (
                                        1.0
                                        if score is True
                                        else (0.0 if score is False else float(score))
                                    )
                                    scores.append(score_val)
                    if scores:
                        avg_score = sum(scores) / len(scores)
                        logger.info(
                            f"  {evaluator_name}: {avg_score:.2%} average ({len(scores)}/{len(experiment_results.results)} evaluated)"
                        )
                ...
                for neg_type, vals in neg_breakdown.items():
                    if vals:
                        logger.info(
                            "    %s: %.2f%% (%s)", neg_type, 100 * sum(vals) / len(vals), len(vals)
                        )
